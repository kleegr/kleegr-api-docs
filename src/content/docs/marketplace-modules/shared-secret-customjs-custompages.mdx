...
description: "User Context in Marketplace Apps — Kleegr developer documentation"
---

Kleegr provides a secure mechanism for accessing authenticated user information through signed tokens. This guide explains how you can generate and use `Shared Secret` key to access user context in secured manner.
...
If you're using custom JavaScript injected into Kleegr pages, use the `exposeSessionDetails` method:

```javascript function getUserData() {
  try {
...
```javascript CryptoJS = require('crypto-js')
...
[Kleegr Marketplace App Template](https://github.com/GoHighLevel/ghl-marketplace-app-template/tree/main)
